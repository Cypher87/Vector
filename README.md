...
The synchronization panel lists every connected device with its device class, browser, operating system, last activity, and an indication of the current device. Devices can be given a custom name, such as `Living room tablet`; clearing that name restores the automatic browser and operating-system label. Any other device can be disconnected individually; its session is invalidated immediately. Names and device-list changes are synchronized live.
